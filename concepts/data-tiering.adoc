...
= Data tiering add-on service for Keystone
:hardbreaks:
:nofooter:
:icons: font
:linkattrs:
...
[NOTE]
The tiering add-on service is required only when data is tiered to any non-NetApp storage such as Amazon Web Services (AWS) S3, Azure Blob, Google Cloud Platform (GCP), and other Keystone STaaS-supported, S3-compatible, third party object storage. This service is available for unified data storage type only.
