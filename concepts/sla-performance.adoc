...
[.lead]
option 1: ##NetApp Keystone offers latency-based SLO targeting 90^th^ percentile latency per performance service level instance, up to target maximum IOPs, for unified (excluding _Value_ performance service level), block-optimized, and AFX data storage types, as described in the Keystone order for consumed capacity up to the burst limit, according to the following listed terms and conditions.##
...
== Metrics
* *Degraded performance*: The number of minutes per incident when the 90^th^ percentile latency target is not met.
* The *90^th^ percentile latency* is measured per volume, per ##performance service level instance##, for all volumes in a Keystone Order. Latency is sampled every five minutes, and the 90^th^ percentile value calculated over a 24-hour period is used as the daily measure, considering the following points:
...
* ##<must review> <to match option 2:Applicable only to NetApp AFF platforms (for unified and block-optimized data storage types, not including _Value_ performance service level) and AFX platforms (for AFX data storage type), up to target maximum IOPs., or ##
...
a|
*Target 90^th^ percentile latency* |&lt;=1ms |&lt;=1ms |&lt;=4ms |&gt;=4ms
|===
