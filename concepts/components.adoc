...
[.lead]
There are multiple requirements for enabling NetApp Keystone services in your environment.
...
Network connectivity is required between the storage infrastructure installed at customer data center or customer owned co-located services and Keystone operations center. Customer is responsible for providing the compute and virtual machines; and also the internet services. The network design will be over a secured protocol and firewall policies will be approved by both NetApp and customers.
