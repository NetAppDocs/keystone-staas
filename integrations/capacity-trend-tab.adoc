...
##You can view daily accrued burst data usage for a monthly or quarterly billing period. When you click the bar that displays the invoiced data, the Billable Provisioned Capacity section will appear below the bar chart, offering you *Graph* and *Table* viewing options. The default graph view displays daily accrued burst data usage in a line graph format, showing changes in usage over time.##
