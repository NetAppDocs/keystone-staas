...
image:aiq-ks-subtime-4.png[Capacity Trend tab with details]
...
##Indicator of the total burst capacity (in TiB) accumulated during each 2-minute interval within a month, for the current billing cycle. The accrued burst usage for an entire month is calculated as this: `[sum of bursts in month /(days in month)*24*60]*interval duration`.

You can calculate the accrued burst for brief periods, such as every two minutes, using this: `[burst/(days in month)*24*60]*interval duration`.The burst is the difference between the consumed capacity and the committed capacity. For example, if the consumed capacity is 120 TiB and the committed capacity is 100 TiB, the burst capacity would be 20 TiB, resulting in an accrued burst usage of 0.000925926 TiB for a 30-day month period.##
