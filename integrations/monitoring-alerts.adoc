...
+
image:monitoring-alerts-default-view-2.png[A screenshot of active alerts for Keystone subscriptions]
+
You can view alert details such as severity, ID, type and condition, description, triggered time, subscription number, tracking ID, performance service level, and status. The page also shows the total unresolved alerts by severity and type.
+
...
* *Review and resolve alerts*: ##Select the alert ID to open a detailed view of the alert. This view displays the alert description and suggested resolution steps. From this detailed view, you can mark the alert as resolved using the *Edit* button or add supporting notes using the *Add note* button.##
+
##You can also use the ellipsis icon next to the *Status* column to perform the following actions:##
+
** *Edit*: ##Opens a pop-up to change the alert status and add resolution details.##
** *Add note*: ##Opens a pop-up to add a note to the alert.##
** *View*: ##Opens the same detailed view as selecting the alert ID.##
+
NOTE: Editing alerts requires the *Keystone admin* role. The *Keystone viewer* role can only view alert details. To learn more, refer to link:https://docs.netapp.com/us-en/console-setup-admin/reference-iam-keystone-roles.html[Keystone access roles in NetApp Console^].
