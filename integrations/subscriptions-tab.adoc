...
+
image:console-subscription.png[view the subscription tab details]
+
...
+
* *Subscription number*: The subscription number of the Keystone subscription assigned by NetApp. Subscriptions with the image:metrocluster-icon.png[MetroCluster subscription symbol] icon are MetroCluster subscriptions. To learn more about MetroCluster subscriptions, refer to link:../integrations/adp-tab.html[View consumption and health of your Keystone MetroCluster subscriptions].
...
* *Linked subscriptions*: If you have any secondary subscriptions linked to your primary subscription, this column lists the linked subscription numbers for the primary subscription. This column is hidden by default and is visible only when linked subscriptions are available.
...
* *Days to subscription expiry*: ##Displays an icon alongside the expiration date to indicate how close the subscription is to expiring and whether it will renew automatically. This status is read-only and reflects your contract terms. To review contract details, refer to link:../integrations/subscription-timeline.html[View the timeline of your Keystone subscriptions].##
+
##When auto-renewal is off, the icon color indicates expiry proximity:##
+
** *Green*: More than 120 days remain until expiry.
** *Orange*: Between 91 and 120 days remain until expiry.
** *Red*: Fewer than 90 days remain until expiry. To avoid service disruption, submit a renewal request to NetApp Keystone support.
** *Gray*: The subscription has expired.
+
##For subscriptions with auto-renewal on or a month-on-month billing cycle, an auto-renewal icon is displayed. For month-on-month subscriptions, this indicates the subscription has no fixed end date and renews automatically every month until cancelled. For other subscriptions, it indicates the subscription will renew automatically at the end of the current term.##
+
NOTE: You can click the down arrow image:down-arrow.png[expand icon] in the table header to expand all subscriptions and view the performance service levels information for each subscription simultaneously. This shows detailed capacity information including current usage, committed capacity, provisioned capacity, burst capacity, and available capacity for all subscriptions. Alternatively, you can click the down arrow next to the *Expiration date* column in individual subscription rows to expand them one at a time.
+
...
* *Linked Subscriptions*: If you have any secondary subscriptions linked to your primary subscription, this column lists the linked subscription numbers for the primary subscription.
