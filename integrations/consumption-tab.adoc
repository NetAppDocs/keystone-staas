...
+
NOTE: ##If you select a date range of fewer than 30 days, the chart displays one data collection point per day for only the selected days. For example, a 4-day range shows 4 data collection points, not 30. The level of detail shown at the bottom of the chart also varies by range: for periods of 30 days or fewer, both the date and time are displayed for each data point; for periods greater than 30 days, only the date is displayed. For a more granular view of capacity usage within any interval, you can use the zoom-in feature described below.##
+
