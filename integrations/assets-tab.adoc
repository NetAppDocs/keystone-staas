...
[.lead]
You can view detailed information about ONTAP clusters and nodes, and StorageGRID grids, sites, and nodes, managed by a single Keystone subscription. You can access these subscription assets in the Keystone dashboard from the NetApp Console or Digital Advisor. 
...
* The *View* dropdown options depend on your Keystone subscription platforms. *Volumes in clusters* and *Nodes in clusters* are available for ONTAP. For StorageGRID, the *View* dropdown is not available, the *Nodes in grids* table displays automatically. If your subscription includes both platforms, all three options are available.
====
...
+
You see the detailed information about volumes in clusters including volume name, node serial number, cluster name, host name, and performance service levels. You can also monitor the provisioned capacity, logical and physical utilization, cold data, and hot data. 
+
NOTE: The *Cold tier %* and *Hot tier %* columns show the percentage of data on each tier, calculated as (Cold data / Total footprint) × 100 and (Hot data / Total footprint) × 100 respectively.
+
