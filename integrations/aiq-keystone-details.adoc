...
. Select the time range from the calendar icons in the *From Date* and *To Date* fields. Only dates until the subscription end date can be selected. The default time range is from the start date of the billing period to the current date.
+
[NOTE]
...
+
* *Current Consumed*: Indicator for the consumed capacity (in TiB) defined for the service level. You cannot see this indicator if the current consumption is in burst. This field uses specific colors for usage:
** Grey: None.
