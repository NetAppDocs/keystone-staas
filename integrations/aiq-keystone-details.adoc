...
.##*MetroCluster configurations: Additional details for protected clusters*##
[%collapsible]
====
If the clusters in your ONTAP storage environment are configured in a MetroCluster setup, the consumption data of your Keystone subscription is split in the same time series chart to display the consumption at the primary and mirror sites. 

[NOTE]
The splitting of the consumption bar charts for the cluster peers in a MetroCluster configuration occurs for only base service levels. For Advanced Data Protection service levels, this demarcation does not appear, and the charts appear as in a regular ONTAP non-MetroCluster configuration.

In the following image, you can see the same time series chart marks off the mirror site consumption in a lighter shade of the color code used for the primary site.

image:mcc-chart.png[]

The tool-tip on mouse hover displays the consumption breakdown (in TiB) for the respective primary and mirror sites.

image:mcc-pop-up.png[]
...
+
The volume details are displayed. You can scroll across the columns and learn more about them by hovering your mouse on the information icons beside the column headings. You can sort by the columns and filter the lists to view specific information. ##For MetroCluster configurations, an additional column appears to indicate whether the volume is a primary or mirror volume in the high-availability (HA) cluster pair.## You can copy individual node serial numbers by clicking the *Copy Node Serials* button. 
