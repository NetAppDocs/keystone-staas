...
[NOTE]
The splitting of the consumption bar charts for the cluster peers in a MetroCluster configuration occurs for only base service levels. For Advanced Data Protection service levels, this demarcation does not appear, and the charts appear as in a regular non-MetroCluster ONTAP configuration.

In the following image, you can see a same time series chart marks off the consumption at the mirror site in a lighter shade of the same color code used for the primary site.

image:mcc-chart.png[]

The tool-tip on mouse hover displays the consumption breakdown at the respective primary and mirror sites in TiB.
...
+
The volume details are displayed. You can scroll across the columns and learn more about them by hovering your mouse on the information icons beside the column headings. You can sort by the columns and filter the lists to view specific information. For MetroCluster configurations, an additional column appears to indicate whether the volume occurs in a primary or mirror site in the MetroCluster setup. You can copy individual node serial numbers by clicking the *Copy Node Serials* button. 
