...
[%collapsible]
====
If you have subscribed to the Advanced Data Protection (ADP) add-on service, you can view the breakup of the consumption data according to the MetroCluster primary and mirror sites on the *Subscription Time Series* tab. 

For information about Advanced Data Protection, see link:../concepts/adp.html[Advanced Data Protection].
...
[NOTE]
The consumption bar charts are split for only base service levels. For ADP service levels, this demarcation does not appear.

.Advanced data protection service levels

For ADP service levels, the total consumption is split, and the usage at each site is reflected and billed in a separate subscription; that is one subscription for the primary site, and another for the mirror site. That is why, when you select the subscription number for the primary site on the *Subscription Time Series* tab, the consumption charts for the ADP service levels display the discrete consumption details for only the primary site. Because each site in a MetroCluster configuration acts as a source and a mirror, for the ADP service levels, the consumption at each site includes the source and the mirror volumes created at that site.

.Base service levels

For the base service levels, however, the volumes are charged as provisioned at the primary and mirror sites, and hence the same bar chart is split according to the consumption at the primary and mirror sites.
...
For the _Advanced Data Protect Extreme_ service level, the charts appear as this:

image:adp-src.png[]

.What you can see for the secondary (mirror site) subscription

When you check the secondary subscription, you can see the bar chart for the _Extreme_ service level at the same data collection point is reversed, and the consumption breakup at the primary and mirror sites is 1.05 TiB and 1.02 TiB respectively.

image:mcc-chart-mirror.png[]

For the _Advanced Data Protect Extreme_ service level and the same data collection point, the chart appears as this:
...
+
The volume details are displayed. You can scroll across the columns and learn more about them by hovering your mouse on the information icons beside the column headings. You can sort by the columns and filter the lists to view specific information. ##For Advanced Data Protection add-on services, an additional column appears to indicate whether the volume is a primary or mirror volume in the MetroCluster configuration.## You can copy individual node serial numbers by clicking the *Copy Node Serials* button. 
