...
* *End Date*: The end date of the subscription. If a subscription has no end date, and it gets renewed automatically, you see `Month-on-month` instead of the date. Based on this date, you might see notifications for subscriptions that are about to end or have auto-renewal policies attached.
...
. Select the time range from the calendar icons in the *From Date* and *To Date* fields. The default time range is from the start date of the billing period to the current date. You can select a date only up to the subscription end date.
+
[NOTE]
...
+
* *Current Consumed*: Indicator for the consumed capacity (in TiB) defined for the service level. You cannot view the current consumed capacity if the consumption is in burst. This field uses specific colors for usage:
** Grey: None.
