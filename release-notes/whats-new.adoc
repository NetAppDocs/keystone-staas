...
* In addition, the *Volume Summary* tab is updated to display only base subscriptions, excluding add-on services. For more information, see link:../integrations/volumes-objects-tab.html[Volumes & Objects tab^].
...
=== Enhanced SLA status display for Keystone subscriptions
The *Service Level Agreement* tile within the *Keystone Subscriptions* widget on the Active IQ Digital Advisor is enhanced to provide the status of your subscriptions based on sustainability, performance, or availability criteria as per the service level agreement (SLA) with NetApp. Earlier, you can only view the status of subscriptions based on sustainability criteria defined in your SLA. For more information about service level agreements (SLAs), refer to link:..concepts/sla-availability.html[Keystone STaaS SLA].
