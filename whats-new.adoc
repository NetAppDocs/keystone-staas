...
##The Digital Advisor dashboard now includes a new *Total Footprint* column in the *Volume Details* tab within the *Volumes & Objects* tab. This column shows the total logical footprint for volumes using FabricPool tiering, including data in performance and cold tiers, so you can accurately calculate Keystone consumption.##
